...
Runs the **QAOA** (Quantum Approximate Optimisation Algorithm) on **IQM Resonance** quantum
hardware using **CUDA-Q**, with **Readout Error Mitigation (REM)** and automated orchestration
via **Prefect 3.x Cloud**.
...
3. Calibrates the device's readout noise (`rem.calibrate`)
4. Runs a QAOA variational loop optimised with COBYLA, applying REM every iteration (`rem.apply`)
5. Runs a final high-shot execution at optimal parameters
6. Reports the best solution found and the approximation ratio as Prefect artifacts
...
Once your REM looks good in Jupyter, copy your updated `rem.py` into the repo folder and run
...
1. Copy your updated `rem.py` into the repository
2. Push to GitHub:
   ```bash
   git add rem.py
   ...
   ```
3. The next Prefect Cloud run will automatically use your updated `rem.py` — no other files need to change
...
Edit `deploy_qaoa.py` and set `WORK_POOL_NAME` to your Prefect Cloud work pool name,
then run:
...
1. Go to **Deployments** → `qaoa-iqm-resonance`
2. Click **Run** → **Custom Run**
3. Override any parameters in the panel (n_qubits, shots, p_layers, etc.)
4. Click **Submit**
5. Monitor in **Flow Runs** → check the **Artifacts** tab for charts and results
...
In Prefect Cloud runs, the IQM token is loaded automatically from the
`iqm-resonance-token` Secret block. No environment variables needed.
...
`deploy_qaoa.py` registers the flow with Prefect Cloud and points it at the GitHub repo.
`qaoa_iqm_pipeline.py` defines all pipeline stages and imports `calibrate` and `apply`
directly from `rem.py`. When you update `rem.py`, the pipeline automatically uses the
new algorithm on the next run — no other files need to change.
